...
if (Test-Path "packaging\seed\themes\Default") {
    Remove-Item -Recurse -Force "packaging\seed\themes\Default"
}
Copy-Item -Recurse -Force "themes\Default" "packaging\seed\themes\Default"
Copy-Item -Force "config.yaml" "packaging\seed\config.yaml"
if (-not (Test-Path "packaging\fonts\jetbrains-mono\JetBrainsMono-Bold.ttf")) {
    throw "Missing packaging\fonts\jetbrains-mono (required for build)."
}
