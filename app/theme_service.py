...
from app.paths import ensure_user_data, is_frozen, resource_dir, themes_dir

# Dev-only: also list vendor themes when not frozen
VENDOR_THEMES = (
    resource_dir()
    / "vendor"
...
def theme_dirs() -> list[Path]:
    dirs: list[Path] = [product_themes_dir()]
    if not is_frozen() and VENDOR_THEMES.is_dir():
        dirs.append(VENDOR_THEMES)
    return dirs


def list_themes(*, product_only: bool = False) -> list[str]:
    names: set[str] = set()
    bases = [product_themes_dir()] if (product_only or is_frozen()) else theme_dirs()
    for base in bases:
        if not base.is_dir():
            continue
    ...
    product = {
        c.name
        for c in product_themes_dir().iterdir()
        if c.is_dir() and (c / "theme.yaml").is_file()
    }
    ordered = sorted(product) + sorted(names - product)
    return ordered
...
def save_theme(name: str, data: dict[str, Any], *, product_only: bool = True) -> Path:
    """Save theme under user themes/ (never overwrite vendor in place)."""
